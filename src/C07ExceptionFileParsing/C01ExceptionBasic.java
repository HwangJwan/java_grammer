...
//        예외처리 목적 : 1) 사용자에게 적절한 에러 메시지 전달 2) 에외 발생시 디버깅을 위한 로그 기록
        System.out.println("계산기 프로그램입니다. ");
        Scanner sc = new Scanner(System.in);
        System.out.println("분자를 입력해주세요");
        try {
            int head = Integer.parseInt(sc.nextLine());
            System.out.println("분모를 입혁해주세요");
...
//            시스템에 디버깅을 위한 로그 자세히 기록
            ...
            e.printStackTrace();
//            Exception은 모든 예외 클래스의 조상 클래스. 구체적인 에외 클래스가 catch부에 있다면, 해당 부분으로 우선 분기처리.
        }catch (Exception e) {
...
//        예외처리를 하지 않을 경우 프로그램은 강제종료. 예외처리를 할 경우 프로그램은 예외가 발생하더라도 지속.
