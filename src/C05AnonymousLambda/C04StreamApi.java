...
//        }
//
////        함수형 프로그래밍 방식 : 데이터와 객체 중심이 아닌, 입력에 따른 출력(연산 작업 포함)만 존재하는 코딩스타일
...
//        int accSMultiply=Arrays.stream(intArr).reduce(1,(a,b)->a*b);
//        System.out.println(accSMultiply);
//        String[] stArr={"java","python","c++"};
...
//        제네릭의 타입 소거로 인해, toArray를 통해 새로운 String배열을 곧바로 만들어네는 것은 불가
...
//
////        메소드 참조 : 하나의 메서드만을 호출하는 경우에 매개변수를 제거한 방식
...
//
////        1) 모든 객체의 평균나이(average)
//        double averageAge=list.stream().mapToInt(a->a.getAge()).average().getAsDouble();
//        System.out.println(averageAge);
//
////        2) 정렬을 통한 가장 나이 어린 사람 찾기(sorted+findFirst)
//        System.out.println(list.stream().sorted((o1, o2) -> o1.getAge()-o2.getAge()).findFirst().toString());
//
////        3) 30대인 사람들의 이름만 모아서 새로운 String배열에 담기(map 활용)
//        String[] answer=list.stream().filter(a->a.getAge()>=30).map(a->a.getName()).toArray(a->new String[a]);
//        Arrays.stream(answer).forEach(a-> System.out.println(a));

//        Optional 객체 : 특정 객체의 값이 없을지도(null) 모른다는 것을 명시적으로 표현한 객체
//        String st1=null;
//        if(st1 == null) {
...
////        방법 1. isPresent()로 확인후에 get()
//        if(opt1.isPresent()) {
//            System.out.println(opt1.get());
//        }
//        else {
////            System.out.println(opt1.get()); // error
//            System.out.println("값이 없습니다.");
//        }
...
////        방법 4. orElseThrow() - 가장 많이 사용, 값이 있으면 있는 값 return, 없으면 지정한 에외(에러) 강제발생
...
//        System.out.println(opt2_1.get());   // error -의도치 않은 에러(지양)
...
//        optional 객체 처리 실습2)
        ...
        list.add(new Student("lee",35));
        list.add(new Student("park",22));
        System.out.println("조회하고자 하는 student의 index를 입력해주세요.");
